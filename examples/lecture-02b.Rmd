...
We can use the `color` argument with a **quoted** input to arbitraility select colors:
...
In addition to creating stacked plots, we can also disaggregate our data so that we get a series of bars for each `fill` category within a given `x` category. This is done with the `position = "dodge"` argument:
...
This helps us see that an overwhelming number of cars that are SUVs have two particular typoes of transmissions, for example.

We can also use a different type of position adjustment to reveal "missing" points on a scatterplot. A normal scatter plot stacks identical combinations of x,y pairs in a way that makes the data appear less numerous than they actually are. For example:
